...
export const ChatView: React.FC<ChatViewProps> = ({ user, addToast, initialRecipient }) => {
    ...
    const fetchData = useCallback(async (isInitial = false) => {
        ...
        try {
            ...
            if(initialRecipient && isInitial) {
                const existingConvo = sortedConvos.find((c: any) => c.participants.includes(initialRecipient.id));
                if (existingConvo) {
    ...
    useEffect(() => {
        const handleNewMessage = (message: ChatMessage) => {
            // Update message list if it's for the active conversation
            ...
            setConversations(prev => {
                ...
                // If it's a new conversation, we might need to fetch it
                fetchData(); 
                return prev;
            });
        };

        socketService.on<ChatMessage>('message:new', handleNewMessage);

        return () => {
            socketService.off('message:new');
        };
    }, [activeConversationId, fetchData]);

    useEffect(() => {
        const fetchMessages = async () => {
            if (activeConversationId && !String(activeConversationId).toString().startsWith('temp_')) {
                try {
    ...
    const handleSendMessage = async (e: React.FormEvent) => {
        ...
        if (!recipient) {
            addToast("Cannot find recipient.", "error");
            return;
        }
        ...
        try {
            // Send via API for persistence, backend will broadcast via WebSocket
            await api.sendMessage(user.id, recipient.id, newMessage);
        } catch (error) {

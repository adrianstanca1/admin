...
export const ProjectsView: React.FC<ProjectsViewProps> = ({ user, addToast, onSelectProject }) => {
    ...
    const filteredAndSortedProjects = useMemo(() => {
        const filtered = allProjects.filter(project => {
            ...
            const matchesSearch = project.name.toLowerCase().includes(lowerCaseSearchTerm) ||
                                  project.location.address.toLowerCase().includes(lowerCaseSearchTerm);
        ...
        <div className="space-y-6">
            {isCreateModalOpen && (
                <CreateProjectModal
                    ...
                    onProjectCreated={(newProject) => {
                        ...
                        setSelectedProjectForModal(newProject);
                    }}
                    addToast={addToast}
                />
            )}
            ...
            {selectedProjectForModal && (
                <ProjectDetailModal
                    ...
                    addToast={addToast}
                    openTaskCount={projectStats.get(selectedProjectForModal.id)?.taskCount || 0}
                />
            )}
                    ...
                    {filteredAndSortedProjects.map(project => {
                        ...
                        return (
                            <ProjectSummaryCard
                                ...
                                health={health}
                                onSelect={() => setSelectedProjectForModal(project)}
                            />
                        );
                    })}
                </div>

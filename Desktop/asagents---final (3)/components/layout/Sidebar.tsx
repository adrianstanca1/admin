...
const NavItem: React.FC<NavItemProps> = ({ icon, label, view, activeView, setActiveView, badgeCount }) => (
    <button
        ...
        className={`w-full group flex items-center justify-between gap-3 px-3 py-2 rounded-md text-sm transition-colors relative ${
            activeView === view
                ? 'bg-primary/10 text-primary font-semibold'
                : 'text-muted-foreground hover:bg-black/5 hover:text-foreground'
        }`}
    >
        {activeView === view && <div className="absolute left-0 top-1 bottom-1 w-1 bg-primary rounded-r-full" />}
        <div className="flex items-center gap-3">
            {icon}
            <span>{label}</span>
        </div>
        {badgeCount && badgeCount > 0 && (
            <span className={`ml-auto text-white text-xs font-bold px-2 py-0.5 rounded-full ${activeView === view ? 'bg-primary' : 'bg-secondary'}`}>
...
const NavGroup: React.FC<{ title: string; children: React.ReactNode }> = ({ title, children }) => {
    ...
    return (
        <div>
            <button onClick={() => setIsOpen(!isOpen)} className="w-full flex justify-between items-center px-3 py-2 text-xs font-bold uppercase text-muted-foreground/60 hover:text-muted-foreground">
                {title}
                <svg xmlns="http://www.w3.org/2000/svg" className={`h-4 w-4 transition-transform ${isOpen ? 'rotate-90' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7" /></svg>
            </button>
            {isOpen && <div className="pl-2 space-y-1 mt-1">{children}</div>}
        </div>
    );
}
...
export const Sidebar: React.FC<SidebarProps> = ({ user, activeView, setActiveView, onLogout, pendingTimesheetCount, openIncidentCount, unreadMessageCount }) => {
    ...
    const commonProps = { activeView, setActiveView };
    ...
    const mainNav = [
        ...
        { view: 'time', label: 'Time Clock', icon: <svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>, permission: hasPermission(user, Permission.SUBMIT_TIMESHEET)},
    ...
    return (
        <aside className="w-64 sidebar-pane p-4 flex flex-col flex-shrink-0 h-screen z-20">
            ...
            <nav className="flex-grow space-y-1 overflow-y-auto">
                {user.role === Role.PRINCIPAL_ADMIN ? (
                    ...
                    <>
                        {mainNav.length > 0 && <NavGroup title="Main">{mainNav.map(item => <NavItem key={item.view} {...item} {...commonProps} />)}</NavGroup>}
                        {projectNav.length > 0 && <NavGroup title="Projects">{projectNav.map(item => <NavItem key={item.view} {...item} {...commonProps} />)}</NavGroup>}
                        {managementNav.length > 0 && <NavGroup title="Management">{managementNav.map(item => <NavItem key={item.view} {...item} {...commonProps} />)}</NavGroup>}
                        
                        <NavGroup title="Tools">
                            <NavItem view="tools" label="AI Tools" icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9.663 17h4.673M12 3v1m6.364 1.636l-.707.707M21 12h-1M4 12H3m3.343-5.657l-.707-.707m2.828 9.9a5 5 0 117.072 0l-.548.547A3.374 3.374 0 0014 18.469V19a2 2 0 11-4 0v-.531c0-.895-.356-1.754-.988-2.386l-.548-.547z" /></svg>} {...commonProps} />
                        </NavGroup>
                    </>
                )}
            </nav>
            <div className="mt-auto pt-4 border-t border-border">
                 <NavItem icon={<svg xmlns="http://www.w3.org/2000/svg" className="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10.325 4.317c.426-1.756 2.924-1.756 3.35 0a1.724 1.724 0 002.573 1.066c1.543-.94 3.31.826 2.37 2.37a1.724 1.724 0 001.065 2.572c1.756.426 1.756 2.924 0 3.35a1.724 1.724 0 00-1.066 2.573c.94 1.543-.826 3.31-2.37 2.37a1.724 1.724 0 00-2.572 1.065c-.426 1.756-2.924 1.756-3.35 0a1.724 1.724 0 00-2.573-1.066c-1.543.94-3.31-.826-2.37-2.37a1.724 1.724 0 00-1.065-2.572c-1.756-.426-1.756-2.924 0 3.35a1.724 1.724 0 001.066-2.573c-.94-1.543.826-3.31 2.37-2.37.996.608 2.296.07 2.572-1.065z" /><path strokeLinecap="round" strokeLinejoin="round" d="M15 12a3 3 0 11-6 0 3 3 0 016 0z" /></svg>}
                    label="Settings" view="settings" {...commonProps} />
                <button
                    onClick={onLogout}
                    className="w-full flex items-center gap-3 px-3 py-2 rounded-md text-sm text-muted-foreground hover:bg-black/5 hover:text-foreground mt-1"
                >

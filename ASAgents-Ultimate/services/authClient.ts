...
let defaultBaseUrl = sanitizeBaseUrl(initialEnvironment.apiUrl);
...
console.log('[authClient] Configuration initialized:', {
    apiUrl: initialEnvironment.apiUrl,
    sanitizedBaseUrl: defaultBaseUrl,
    allowMockFallback,
    mode: defaultBaseUrl ? 'backend' : 'mock'
});
...
export const resetAuthClient = () => {
    ...
    defaultBaseUrl = sanitizeBaseUrl(refreshed.apiUrl);
...
// Helper function to determine tenant from email domain
const getTenantFromEmail = (email: string): string => {
    const domain = email.split('@')[1]?.toLowerCase();

    // Map email domains to tenant slugs
    const domainToTenant: Record<string, string> = {
        'demo-construction.com': 'demo-construction',
        'buildright.com': 'buildright',
        'gmail.com': 'demo-construction', // Default for demo purposes
    };

    return domainToTenant[domain] || 'demo-construction'; // Default tenant
};

export const authClient = {
    login: (credentials: LoginCredentials): Promise<LoginResult> => {
        // Add tenant to credentials if not provided
        const enhancedCredentials = {
            ...credentials,
            tenant: credentials.tenant || getTenantFromEmail(credentials.email)
        };

        return withAuthFallback<LoginResult>(
            '/auth/login',
            { method: 'POST', body: JSON.stringify(enhancedCredentials) },
            () => authApi.login(credentials)
        );
    },

    register: (payload: RegistrationPayload): Promise<AuthenticatedSession> => {
        // Add tenant to registration payload if not provided
        const enhancedPayload = {
            ...payload,
            tenant: payload.tenant || (payload.email ? getTenantFromEmail(payload.email) : 'demo-construction')
        };

        return withAuthFallback<AuthenticatedSession>(
            '/auth/register',
            { method: 'POST', body: JSON.stringify(enhancedPayload) },
            () => authApi.register(payload)
        );
    },

    refreshToken: async (refreshToken: string): Promise<{ token: string; refreshToken: string }> => {
        return withAuthFallback<{ token: string; refreshToken: string }>(
            '/auth/refresh',
    ...
    checkEmailAvailability: async (email: string): Promise<EmailAvailability> => {
        return withAuthFallback<EmailAvailability>(
            '/auth/check-email',
    ...
    lookupInviteToken: async (token: string): Promise<InvitePreview> => {
        return withAuthFallback<InvitePreview>(
            '/auth/invite/lookup',
    ...
    me: async (token: string): Promise<{ user: User; company: Company }> => {
        return withAuthFallback<{ user: User; company: Company }>(
            '/auth/me',
            { 
                method: 'GET',

...
import { User, Expense, Invoice, ExpenseStatus, InvoiceStatus } from '../../types';
...
export const FinancialDashboard: React.FC<FinancialDashboardProps> = ({
  ...
  const recentInvoices = invoices
    .sort((a, b) => {
      const dateA = typeof a.issueDate === 'string' ? new Date(a.issueDate) : a.issueDate;
      const dateB = typeof b.issueDate === 'string' ? new Date(b.issueDate) : b.issueDate;
      return dateB.getTime() - dateA.getTime();
    })
    .slice(0, 5);
  ...
  const overdueInvoices = invoices.filter(invoice =>
    invoice.status === InvoiceStatus.SENT && isAfter(new Date(), invoice.dueDate)
  );

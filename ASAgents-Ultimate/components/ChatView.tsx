            ...
            <Card className="w-full max-w-md" onClick={e => e.stopPropagation()}>
                ...
                <div className="space-y-2 max-h-96 overflow-y-auto">
                    {availableUsers.length > 0 ? availableUsers.map(p => (
                        <button type="button" key={p.id} onClick={() => onStartConversation(p)} className="w-full flex items-center gap-3 p-2 rounded-lg hover:bg-slate-100">
...
export const ChatView: React.FC<ChatViewProps> = ({ user, addToast, initialRecipient }) => {
    ...
    useEffect(() => {
        ...
        const fetchMessages = async () => {
            try {
                const newMessages = await api.getMessagesForConversation(activeConversationId);
    ...
    const handleSendMessage = async () => {
        ...
        try {
            const result = await api.sendMessage(user.id, otherParticipantId, optimisticMessage.content);
            const { conversation: updatedConvo, message: sentMessage } = result;
                ...
                <aside className="w-1/3 border-r dark:border-slate-700 flex flex-col">
                    ...
                    <div className="overflow-y-auto">
                        {conversations.map(convo => {
                            ...
                            return (
                                <button type="button" key={convo.id} onClick={() => setActiveConversationId(convo.id)} className={`w-full text-left p-4 flex gap-3 items-center relative ${activeConversationId === convo.id ? 'bg-sky-50 dark:bg-sky-900/50' : 'hover:bg-slate-50 dark:hover:bg-slate-800'}`}>

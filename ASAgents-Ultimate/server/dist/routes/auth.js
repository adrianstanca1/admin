...
router.post('/register', async (req, res) => {
    const { tenant, email, password, firstName, lastName, companyName } = req.body ?? {};
    ...
    try {
        ...
        const [tenantRows] = await pool.query('SELECT id FROM tenants WHERE slug = ? AND is_active = 1', [tenant]);
        if (tenantRows.length === 0) {
            return res.status(404).json({ message: 'Tenant not found' });
        }
        const tenantId = tenantRows[0].id;
        // Check if email already exists
        const [existingUsers] = await pool.query('SELECT id FROM users WHERE email = ? AND tenant_id = ?', [email, tenantId]);
      ...
      ) VALUES (?, ?, ?, ?, ?, ?, ?)`, [tenantId, email, passwordHash, firstName, lastName, 'viewer', 'active']);
        // Get the created user with tenant info
        const [userRows] = await pool.query(`SELECT u.*, t.slug as tenant_slug
       FROM users u

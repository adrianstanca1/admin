...
let managers = null;
export function setManagers(managersInstance) {
...
router.get('/health/detailed', authenticateUser, requireRole(['owner', 'admin']), async (req, res) => {
    try {
        const database = await healthCheck();
        let managersHealth = null;
...
router.get('/config', authenticateUser, requireRole(['owner', 'admin']), async (req, res) => {
    try {
        if (!managers) {
            return res.status(503).json({ message: 'Managers not available' });
        }

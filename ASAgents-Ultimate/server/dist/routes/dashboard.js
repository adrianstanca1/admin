...
router.get('/enhanced', authenticateUser, async (req, res) => {
    try {
        const tenantId = req.user?.tenant_id;
        const userId = req.user?.sub;
        if (!tenantId || !userId) {
...
router.get('/analytics/period', authenticateUser, async (req, res) => {
    try {
        const tenantId = req.user?.tenant_id;
...
router.get('/analytics/user/:userId', authenticateUser, async (req, res) => {
    try {
        const tenantId = req.user?.tenant_id;
        ...
        const metrics = await EnhancedIntegrationService.getUserPerformanceMetrics(tenantId, parseInt(userId));
...
router.get('/realtime/stats', authenticateUser, async (req, res) => {
    try {
...
router.get('/stats/projects', authenticateUser, async (req, res) => {
    try {
        const tenantId = req.user?.tenant_id;
...
// Get task statistics
router.get('/stats/tasks', authenticateUser, async (req, res) => {
    try {
        const tenantId = req.user?.tenant_id;
        const [stats] = await pool.query(`SELECT 
        t.status,
       ...
       WHERE p.tenant_id = ?
       GROUP BY t.status, t.priority`, [tenantId]);
        return res.json(stats);
    }
    catch (error) {
...
router.get('/stats/financial', authenticateUser, async (req, res) => {
    try {
        const tenantId = req.user?.tenant_id;
        // Get project budgets
        const [projectBudgets] = await pool.query(`SELECT 
        COALESCE(SUM(budget), 0) as total_budget,
        COUNT(*) as project_count
       FROM projects 
       WHERE tenant_id = ?`, [tenantId]);
        // Get expense totals
        const [expenseTotals] = await pool.query(`SELECT 
        e.status,
        COUNT(*) as count,

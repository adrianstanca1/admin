...
router.get('/reports/:reportType', authenticateUser, async (req, res) => {
    try {
        ...
        const report = await AnalyticsService.generateAnalyticsReport(tenantId, reportType, timeframe);
...
router.get('/predictive/project-completion', authenticateUser, async (req, res) => {
    try {
        const tenantId = req.user?.tenant_id;
        if (!tenantId) {
            return res.status(400).json({ message: 'Invalid user context' });
...
router.get('/realtime/metrics', authenticateUser, async (req, res) => {
    try {
        const tenantId = req.user?.tenant_id;
...
router.get('/custom-range', authenticateUser, async (req, res) => {
    try {
        const tenantId = req.user?.tenant_id;
        ...
        const analytics = await EnhancedIntegrationService.getAnalyticsForPeriod(tenantId, startDate, endDate);
        // If specific metrics requested, filter the data
        let filteredAnalytics = analytics;
        if (metrics && typeof metrics === 'string') {
            const requestedMetrics = metrics.split(',');
            filteredAnalytics = analytics.filter((item) => requestedMetrics.some(metric => item.entity_type === metric ||
...
router.get('/user-performance/:userId?', authenticateUser, async (req, res) => {
    try {
        const tenantId = req.user?.tenant_id;
...
// Get trend analysis
router.get('/trends/:entityType', authenticateUser, async (req, res) => {
    try {
        const tenantId = req.user?.tenant_id;
        ...
        const analytics = await EnhancedIntegrationService.getAnalyticsForPeriod(tenantId, startDate.toISOString().split('T')[0], endDate.toISOString().split('T')[0]);
        // Group data by granularity and entity type
        const trendData = analytics
            .filter((item) => item.entity_type === entityType.slice(0, -1)) // Remove 's' from plural
            .reduce((acc, item) => {
        ...
        const trendArray = Object.values(trendData).sort((a, b) => new Date(a.date).getTime() - new Date(b.date).getTime());
        ...
        const earlierAvg = earlierData.reduce((sum, item) => sum + item.count, 0) / earlierData.length;
...
router.get('/compare', authenticateUser, async (req, res) => {
    try {
        const tenantId = req.user?.tenant_id;
        const { period1_start, period1_end, period2_start, period2_end, metrics = 'projects,tasks,expenses' } = req.query;
        if (!tenantId) {
            return res.status(400).json({ message: 'Invalid user context' });
        }
        ...
        const [period1Data, period2Data] = await Promise.all([
            EnhancedIntegrationService.getAnalyticsForPeriod(tenantId, period1_start, period1_end),
            EnhancedIntegrationService.getAnalyticsForPeriod(tenantId, period2_start, period2_end)
        ]);
        const requestedMetrics = metrics.split(',');
        // Calculate comparative metrics
        const comparison = requestedMetrics.map(metric => {
            const period1Count = period1Data.filter((item) => item.entity_type === metric.slice(0, -1) // Remove 's' from plural
            ).length;
            const period2Count = period2Data.filter((item) => item.entity_type === metric.slice(0, -1)).length;

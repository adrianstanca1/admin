...
export class AdminUserService {
     ...
     */
    static async createPrincipalAdmin() {
        ...
        try {
            ...
            const [existingRows] = await pool.query('SELECT id FROM admin_users WHERE email = ?', [email]);
          ...
          permissions, is_active, created_at, updated_at
            ...
            // Log the creation
            await this.logAdminAction(adminId, 'create_principal_admin', 'admin_user', adminId.toString(), null, { email, role: 'super_admin' }, null, null);
    ...
    static async authenticateAdmin(email, password, ipAddress, userAgent) {
        try {
            ...
            if (!passwordValid) {
                ...
                await this.logAdminAction(admin.id, 'failed_login', 'admin_user', admin.id.toString(), null, { email, reason: 'invalid_password' }, ipAddress || null, userAgent || null);
                return { success: false, message: 'Invalid credentials' };
            }
            ...
            await this.logAdminAction(admin.id, 'successful_login', 'admin_user', admin.id.toString(), null, { email }, ipAddress || null, userAgent || null);
    ...
    static async incrementFailedAttempts(adminId) {
       ...
       SET failed_login_attempts = failed_login_attempts + 1,
           locked_until = CASE 
             ...
             ELSE locked_until
           END
       WHERE id = ?`, [this.MAX_LOGIN_ATTEMPTS, this.LOCKOUT_DURATION * 1000, adminId]);
    }
    /**
     * Reset failed login attempts
     */
    static async resetFailedAttempts(adminId) {
        await pool.query('UPDATE admin_users SET failed_login_attempts = 0, locked_until = NULL WHERE id = ?', [adminId]);
    }
    /**
     * Log admin action for audit trail
     */
    static async logAdminAction(adminUserId, action, resourceType, resourceId, tenantId, details, ipAddress, userAgent) {
        try {
            await pool.query(`INSERT INTO admin_audit_log (
     ...
     */
    static async getAdminAuditLogs(limit = 100, offset = 0, adminId, action, tenantId) {
        let query = `

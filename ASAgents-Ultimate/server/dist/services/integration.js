...
export class IntegrationService {
     ...
     */
    static async getDashboardData(tenantId, userId) {
        try {
            // Get recent projects
         ...
         LIMIT 10`, [tenantId]);
            // Get recent tasks
            const [tasks] = await pool.query(`SELECT t.*, 
         ...
         LIMIT 20`, [tenantId]);
            // Get recent expenses
            const [expenses] = await pool.query(`SELECT e.*, 
                p.name as project_name,
         ...
         LIMIT 15`, [tenantId]);
            // Get team members
            const [team] = await pool.query(`SELECT id, email, first_name, last_name, role, is_active, last_login_at, created_at
         FROM users
         WHERE tenant_id = ? AND is_active = TRUE
         ORDER BY first_name, last_name`, [tenantId]);
            // Get recent notifications
            const [notifications] = await pool.query(`SELECT id, type, title, message, is_read, created_at
         FROM notifications 
         WHERE user_id = ? AND tenant_id = ?
         ORDER BY created_at DESC
         LIMIT 10`, [userId, tenantId]);
     ...
     */
    static async getProjects(tenantId, filters = {}) {
        let query = `
      SELECT p.*, 
             COUNT(t.id) as task_count,
             SUM(CASE WHEN t.status = 'done' THEN 1 ELSE 0 END) as completed_tasks,
     ...
     */
    static async calculatePortfolioSummary(tenantId) {
       ...
       WHERE tenant_id = ?`, [tenantId]);
        const [taskStats] = await pool.query(`SELECT 
       ...
       WHERE p.tenant_id = ?`, [tenantId]);
        const [expenseStats] = await pool.query(`SELECT 
     ...
     * Generate operational insights
     */
    static async generateOperationalInsights(tenantId) {
        const portfolioSummary = await this.calculatePortfolioSummary(tenantId);
        return {
     ...
     */
    static generateRecommendations(portfolioSummary) {

...
export class TimeTrackingService {
     ...
     */
    static async startTimeEntry(tenantId, userId, data) {
        try {
            // Stop any currently running entries for this user
            await this.stopAllRunningEntries(tenantId, userId);
            const [result] = await pool.query(`INSERT INTO time_entries (
     ...
     * Stop a time entry
     */
    static async stopTimeEntry(tenantId, timeEntryId, userId) {
        try {
            // Calculate duration and update entry
            await pool.query(`UPDATE time_entries 
         SET end_time = NOW(), 
             duration = TIMESTAMPDIFF(MINUTE, start_time, NOW()),
             is_running = 0,
             updated_at = NOW()
         WHERE id = ? AND tenant_id = ? AND user_id = ?`, [timeEntryId, tenantId, userId]);
     ...
     */
    static async getCurrentTimeEntry(tenantId, userId) {
        try {
            const [rows] = await pool.query(`SELECT te.*, p.name as project_name, t.title as task_title
     ...
     */
    static async getTimeEntries(tenantId, userId, filters = {}) {
        try {
            ...
            // Get total count
            const [countResult] = await pool.query(`SELECT COUNT(*) as total FROM time_entries te ${whereClause}`, params);
     ...
     */
    static async getTimeTrackingSummary(tenantId, userId, startDate, endDate) {
        try {
            // Get basic summary
         ...
         AND DATE(start_time) BETWEEN ? AND ?`, [tenantId, userId, startDate, endDate]);
            // Get project breakdown
            const [projectRows] = await pool.query(`SELECT 
           te.project_id,
         ...
         ORDER BY total_minutes DESC`, [tenantId, userId, startDate, endDate]);
            // Get daily breakdown
            const [dailyRows] = await pool.query(`SELECT 
         ...
         ORDER BY date`, [tenantId, userId, startDate, endDate]);
            ...
            const productivityScore = this.calculateProductivityScore(summary.entries_count, totalHours, dailyBreakdown.length);
            return {
                totalHours,
                billableHours,
                nonBillableHours,
     ...
     */
    static async getProductivityInsights(tenantId, userId, days = 30) {
        try {
            const startDate = new Date();
            startDate.setDate(startDate.getDate() - days);
            // Get hourly productivity data
            const [hourlyRows] = await pool.query(`SELECT 
         ...
         ORDER BY hour`, [tenantId, userId, startDate.toISOString()]);
         ...
         ORDER BY total_minutes DESC`, [tenantId, userId, startDate.toISOString()]);
            ...
            const [sessionRows] = await pool.query(`SELECT AVG(CASE WHEN duration IS NOT NULL THEN duration ELSE 0 END) as avg_session
         FROM time_entries 
         WHERE tenant_id = ? AND user_id = ? 
         AND start_time >= ? AND duration IS NOT NULL`, [tenantId, userId, startDate.toISOString()]);
            ...
            const recommendations = this.generateProductivityRecommendations(peakHours, averageSessionLength, mostProductiveDays);
            return {
     ...
     */
    static async stopAllRunningEntries(tenantId, userId) {
        await pool.query(`UPDATE time_entries 
       SET end_time = NOW(), 
           duration = TIMESTAMPDIFF(MINUTE, start_time, NOW()),
           is_running = 0,
           updated_at = NOW()
       WHERE tenant_id = ? AND user_id = ? AND is_running = 1`, [tenantId, userId]);
    }
    /**
     * Get single time entry
     */
    static async getTimeEntry(tenantId, timeEntryId) {
        const [rows] = await pool.query(`SELECT te.*, p.name as project_name, t.title as task_title
       FROM time_entries te
       LEFT JOIN projects p ON te.project_id = p.id
       LEFT JOIN tasks t ON te.task_id = t.id
       WHERE te.id = ? AND te.tenant_id = ?`, [timeEntryId, tenantId]);
        if (rows.length === 0) {
     ...
     */
    static calculateProductivityScore(entriesCount, totalHours, activeDays) {
     ...
     */
    static calculateFocusScore(averageSessionLength, peakHoursCount) {
     ...
     */
    static generateProductivityRecommendations(peakHours, averageSessionLength, mostProductiveDays) {

...
export class MultitenantService {
     ...
     */
    static async getAllTenants(limit = 50, offset = 0, filters = {}) {
        try {
            ...
            // Get total count
            const [countRows] = await pool.query(`SELECT COUNT(*) as total FROM tenants ${whereClause}`, params);
            const total = countRows[0].total;
            // Get tenants
            const [rows] = await pool.query(`SELECT * FROM tenants ${whereClause} ORDER BY created_at DESC LIMIT ? OFFSET ?`, [...params, limit, offset]);
     ...
     * Create a new tenant
     */
    static async createTenant(request) {
        try {
            // Check if slug is unique
            const [existingRows] = await pool.query('SELECT id FROM tenants WHERE slug = ?', [request.slug]);
            ...
            // Create tenant
            const [result] = await pool.query(`INSERT INTO tenants (
          ...
          created_at, updated_at
            ...
            // Log admin action
            await AdminUserService.logAdminAction(request.createdByAdminId, 'create_tenant', 'tenant', tenantId.toString(), tenantId, { name: request.name, plan: request.plan }, null, null);
     ...
     * Update tenant
     */
    static async updateTenant(tenantId, updates, adminId) {
        try {
            const updateFields = [];
            const params = [];
            ...
            await pool.query(`UPDATE tenants SET ${updateFields.join(', ')} WHERE id = ?`, params);
            // Log admin action
            await AdminUserService.logAdminAction(adminId, 'update_tenant', 'tenant', tenantId.toString(), tenantId, updates, null, null);
     ...
     */
    static async getTenantUsage(tenantId, startDate, endDate) {
    ...
    static async updateTenantUsage(tenantId) {
        try {
            ...
            const [userCount] = await pool.query('SELECT COUNT(*) as count FROM users WHERE tenant_id = ? AND is_active = 1', [tenantId]);
            const [projectCount] = await pool.query('SELECT COUNT(*) as count FROM projects WHERE tenant_id = ?', [tenantId]);
            // Update tenant current usage
            await pool.query(`UPDATE tenants SET 
         current_users = ?, 
         current_projects = ?,
         last_activity_at = NOW(),
         updated_at = NOW()
         WHERE id = ?`, [userCount[0].count, projectCount[0].count, tenantId]);
            // Insert daily usage record
            await pool.query(`INSERT INTO tenant_usage (
          tenant_id, usage_date, active_users, projects_created
        ) VALUES (?, CURDATE(), ?, ?)
        ON DUPLICATE KEY UPDATE 
          active_users = VALUES(active_users),
          projects_created = VALUES(projects_created)`, [tenantId, userCount[0].count, projectCount[0].count]);
        }
        catch (error) {

...
class UserProfileService {
    ...
    async createUserProfile(data) {
        try {
            ...
            const [existingUsers] = await pool.query('SELECT id FROM user_profiles WHERE auth0_id = ? OR email = ?', [data.auth0_id, data.email]);
            ...
            const [result] = await pool.query(`INSERT INTO user_profiles 
    ...
    async getUserProfileById(id) {
        try {
            const [profiles] = await pool.query('SELECT * FROM user_profiles WHERE id = ? AND is_active = 1', [id]);
    ...
    async getUserProfileByAuth0Id(auth0Id) {
        try {
            const [profiles] = await pool.query('SELECT * FROM user_profiles WHERE auth0_id = ? AND is_active = 1', [auth0Id]);
            if (profiles.length === 0) {
                return null;
            }
            return this.formatUserProfile(profiles[0]);
    ...
    async getUserProfileByEmail(email) {
        try {
            const [profiles] = await pool.query('SELECT * FROM user_profiles WHERE email = ? AND is_active = 1', [email]);
            if (profiles.length === 0) {
                return null;
            }
            return this.formatUserProfile(profiles[0]);
    ...
    async updateUserProfile(id, updates) {
        try {
            ...
            await pool.query(`UPDATE user_profiles SET ${updateFields.join(', ')} WHERE id = ?`, updateValues);
    ...
    async updateLastLogin(auth0Id) {
        try {
            await pool.query('UPDATE user_profiles SET last_login_at = NOW() WHERE auth0_id = ?', [auth0Id]);
    ...
    async listUserProfiles(page = 1, limit = 10, tenantId) {
        try {
            ...
            // Get total count
            const [countResult] = await pool.query(`SELECT COUNT(*) as total FROM user_profiles ${whereClause}`, queryParams);
            const total = countResult[0].total;
            // Get profiles
            const [profiles] = await pool.query(`SELECT * FROM user_profiles ${whereClause} ORDER BY created_at DESC LIMIT ? OFFSET ?`, [...queryParams, limit, offset]);
    ...
    async deleteUserProfile(id) {
        try {
            await pool.query('UPDATE user_profiles SET is_active = 0, updated_at = NOW() WHERE id = ?', [id]);
    ...
    async searchUserProfiles(query, tenantId, limit = 10) {
        try {
            ...
            const [profiles] = await pool.query(`SELECT * FROM user_profiles ${whereClause} ORDER BY first_name, last_name LIMIT ?`, [...queryParams, limit]);

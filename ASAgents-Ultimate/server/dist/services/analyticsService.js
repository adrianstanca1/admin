...
export class AnalyticsService {
     ...
     */
    static async generateAnalyticsReport(tenantId, reportType, timeframe = 'month') {
        try {
            switch (reportType) {
     ...
     */
    static async generateProjectPerformanceReport(tenantId, timeframe) {
        const dateFilter = this.getDateFilter(timeframe);
        // Get project completion metrics
        const [completionMetrics] = await pool.query(`SELECT 
       ...
       FROM projects 
       WHERE tenant_id = ? AND created_at >= ?`, [tenantId, dateFilter]);
        // Get budget performance
        const [budgetMetrics] = await pool.query(`SELECT 
       ...
       WHERE tenant_id = ? AND created_at >= ?`, [tenantId, dateFilter]);
        ...
        const [taskTrends] = await pool.query(`SELECT 
       ...
       ORDER BY date`, [tenantId, dateFilter]);
        const metrics = [
            {
     ...
     */
    static async generateTeamProductivityReport(tenantId, timeframe) {
        const dateFilter = this.getDateFilter(timeframe);
        // Get team productivity metrics
        const [productivityMetrics] = await pool.query(`SELECT 
         u.id,
       ...
       ORDER BY completed_tasks DESC`, [dateFilter, tenantId]);
        // Get time tracking data
        const [timeData] = await pool.query(`SELECT 
     ...
     */
    static async generateFinancialAnalysisReport(tenantId, timeframe) {
        const dateFilter = this.getDateFilter(timeframe);
        // Get expense analysis
        const [expenseAnalysis] = await pool.query(`SELECT 
         e.category,
       ...
       GROUP BY e.category
       ORDER BY total_amount DESC`, [tenantId, dateFilter]);
        // Get budget vs actual
        const [budgetComparison] = await pool.query(`SELECT 
       ...
       HAVING p.budget > 0
       ORDER BY (COALESCE(SUM(e.amount), 0) / p.budget) DESC`, [tenantId, dateFilter]);
     ...
     */
    static async generateOperationalEfficiencyReport(tenantId, timeframe) {
     ...
     */
    static async generateProjectCompletionModel(tenantId) {

...
export class EnhancedIntegrationService {
     ...
     */
    static async getEnhancedDashboardData(tenantId, userId) {
     ...
     * Get projects with comprehensive metrics
     */
    static async getProjectsWithMetrics(tenantId) {
     ...
     */
    static calculateProjectHealthScore(project) {
        let score = 100;
     ...
     */
    static calculateRiskLevel(healthScore, budgetVariance, overdueTasks) {
     ...
     */
    static calculateScheduleVariance(project) {
        if (!project.end_date)
            return 0;
     ...
     */
    static async getTasksWithMetrics(tenantId) {
        const [rows] = await pool.query(`SELECT
        t.*,
       ...
       WHERE t.tenant_id = ?
       ORDER BY t.updated_at DESC`, [tenantId]);
        return rows.map(row => ({
            id: row.id,
     ...
     */
    static async getExpensesWithMetrics(tenantId) {
        const [rows] = await pool.query(`SELECT
        e.*,
     ...
     * Get team with performance metrics
     */
    static async getTeamWithMetrics(tenantId) {
        const [rows] = await pool.query(`SELECT
        u.*,
     ...
     */
    static async getNotificationsWithPriority(tenantId, userId) {
     ...
     * Calculate portfolio summary
     */
    static async calculatePortfolioSummary(tenantId, projects, tasks, team) {
     ...
     * Generate operational insights
     */
    static async generateOperationalInsights(tenantId, projects, tasks, expenses) {
        const completedTasks = tasks.filter(t => t.status === 'done').length;
        const totalTasks = tasks.length;
     ...
     * Get real-time metrics
     */
    static async getRealTimeMetrics(tenantId) {
        const [activeUsersResult] = await pool.query(`SELECT COUNT(DISTINCT user_id) as active_users
       FROM user_sessions
       WHERE tenant_id = ? AND last_activity > DATE_SUB(NOW(), INTERVAL 15 MINUTE)`, [tenantId]);
        const [todayStatsResult] = await pool.query(`SELECT
     ...
     * Generate predictive analytics
     */
    static async generatePredictiveAnalytics(tenantId, projects, tasks) {
     ...
     */
    static async getAnalyticsForPeriod(tenantId, startDate, endDate) {
        const [rows] = await pool.query(`SELECT
       ...
       ORDER BY date`, [tenantId, startDate, endDate]);
        return rows;
    }
    /**
     * Get performance metrics for user
     */
    static async getUserPerformanceMetrics(tenantId, userId) {
        const [rows] = await pool.query(`SELECT
       ...
       WHERE t.tenant_id = ? AND t.assignee_id = ?`, [tenantId, userId]);
        return rows[0] || {};
    }
}
